...
class View (object):
    ...
    def set_parent_view(self, parent_view):
        ...
        GTK: set_transient_for; Qt: setParent preserving window flags.
        """
        top = self.get_top_widget()
        parent_top = parent_view.get_top_widget()

        def _make_transient(child, parent):
            if child is None or parent is None:
                return
            if hasattr(child, "set_transient_for"):       # GTK
                child.set_transient_for(parent)
            elif hasattr(child, "setParent"):              # Qt
        ...
        if type(top) in (list, tuple):
            for t in top:
                _make_transient(t, parent_top)
        else:
            _make_transient(top, parent_top)
        return
    ...
    def set_transient(self, transient_view):
        ...
        """
        top = self.get_top_widget()
        tv_top = transient_view.get_top_widget()
        ...
        if type(top) in (list, tuple):
            for t in top:
                _make_transient(tv_top, t)
        elif top is not None:
            _make_transient(tv_top, top)
        return
