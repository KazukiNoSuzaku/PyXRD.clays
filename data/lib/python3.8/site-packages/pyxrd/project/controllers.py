...
class ProjectController(ObjectListStoreController):
    ...
    def setup_treeview(self, widget):
        ...
        # Connect click handler for context menu / toggle / move buttons
        widget.clicked.connect(lambda idx: self._on_specimen_tv_clicked(widget, idx))
        widget.customContextMenuRequested.connect(
            lambda pos: self._on_specimen_tv_right_click(widget, pos))
        widget.doubleClicked.connect(
            lambda idx: self._on_specimen_tv_double_click(widget, idx))
    ...
    def import_multiple_specimen(self):
        def on_accept(dialog):
            ...
            parser = getattr(dialog, 'parser', None)
            if parser is None and hasattr(dialog, 'get_filter'):
                flt = dialog.get_filter()
                parser = getattr(flt, 'parser', None)

            task = QtThreadedTaskBox()
                ...
                for filename in filenames:
                    ...
                    with DialogFactory.error_dialog_handler(
                        ...
                        specimens = Specimen.from_experimental_data(
                            filename=filename, parent=self.model, parser=parser)
            ...
            def on_complete(*args, **kwargs):
                ...
                for specimen in status_dict["specimens"]:
                    ...
                    last_specimen = specimen
                if last_specimen is not None:
                    if hasattr(self.treemodel, 'get_index_for_object'):
                        idx = self.treemodel.get_index_for_object(last_specimen)
                        if idx.isValid():
                            tv = self.view["project_specimens"]
                            if tv is not None:
            ...
            # Run task box:
            task.cancelrequested.connect(on_interrupted)
            task.stoprequested.connect(on_interrupted)
    ...
    def add_specimen(self):
        ...
        self.model.specimens.append(specimen)
        if hasattr(self.treemodel, 'get_index_for_object'):
            idx = self.treemodel.get_index_for_object(specimen)
            if idx.isValid():
    ...
    def specimen_tv_toggled(self, cell, path, model, colnr):
        """Toggle handler called by delegate when a check-column cell is clicked."""
        if model is not None:
            # path is a QModelIndex row integer or string in Qt delegates
            try:
                row = int(path)
    ...
    def _on_specimen_tv_clicked(self, tv, index):
        """Handle left-click on specimen treeview."""
        if not index.isValid():
            return
        model = tv.model()
        specimen = index.internalPointer()
        if specimen is None and hasattr(model, 'get_object'):
            specimen = model.get_object(index)
        if specimen is None:
            return
        ...
        column = getattr(col, "colnr", None) if col is not None else None
    ...
    def _on_specimen_tv_right_click(self, tv, pos):
        """Handle right-click on specimen treeview."""
        ...
        self.view.show_specimens_context_menu(pos)

    def _on_specimen_tv_double_click(self, tv, index):
        """Handle double-click on specimen treeview."""
        if not index.isValid():
            return
        model = tv.model()
        specimen = index.internalPointer()
        ...
        col = cols[col_idx] if col_idx < len(cols) else None
        column = getattr(col, "colnr", None) if col is not None else None
        if column == self.treemodel.c_name:
            self.parent.on_edit_specimen_activate(None)

    def objects_tv_selection_changed(self, selected=None, deselected=None):
        ObjectListStoreController.objects_tv_selection_changed(self, selected, deselected)
        self.parent.model.current_specimens = self.get_selected_objects()
        return True
