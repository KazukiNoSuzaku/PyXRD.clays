        ...
        try:
            ...
            if self.num_params == 1:
                ...
                for index, (parx, pary) in enumerate(itertools.combinations(list(range(self.num_params)), 2)):
                    ...
                    # Try to interpolate the data, falling back to simpler methods if needed:
                    ...
                    if len(points_x) > 0:
                        for method in ('cubic', 'linear', 'nearest'):
                            if method == 'cubic' and len(points_x) < 4:
                                continue
                            if method == 'linear' and len(points_x) < 3:
                                continue
                            try:
                                zi = griddata((points_x, points_y), points_z, (xi[None, :], yi[:, None]), method=method)
            ...
            logger.error("Unhandled exception while generating parameter space images:")
