...
class RefinementController(DialogController):
    ...
    def setup_refinables_tree_view(self, store, widget):
        ...
        # Labels as text (cross-platform) or pixmap
        def get_label(column, cell, model, itr, user_data=None):
            if hasattr(itr, 'row'):
                from PySide6.QtCore import Qt as _Qt
                obj = model.data(model.index(itr.row(), 0), _Qt.UserRole)
            elif hasattr(model, 'get_tree_node_object'):
                obj = model.get_tree_node_object(itr)
            else:
                obj = None
            if obj is not None:
                text = getattr(obj, 'text_title', str(obj))
                try:
                    cell.set_property("text", text)
                except Exception:
                    cell._rendered_text = text
        ...
        def get_value(column, cell, model, itr, *args):
            ...
            try:
                if hasattr(itr, 'row'):
                    from PySide6.QtCore import Qt as _Qt
                    value = model.data(model.index(itr.row(), col), _Qt.DisplayRole)
                else:
        ...
        widget.append_column(new_text_column(
            ...
            edited_callback=(on_float_edited, (store, store.c_value,)),
            **def_float_args
        ))
        widget.append_column(new_text_column(
            "Min", markup_col=store.c_value_min,
            edited_callback=(on_float_edited, (store, store.c_value_min,)),
            **def_float_args
        ))
        widget.append_column(new_text_column(
            "Max", markup_col=store.c_value_max,
            edited_callback=(on_float_edited, (store, store.c_value_max,)),
            **def_float_args
        ))
    ...
    def _update_method_options_store(self):
        ...
        if tv is None:
            return None
        store = QStandardItemModel()
        store.setHorizontalHeaderLabels(["option", "description"])
        method = self.model.get_refinement_method()
        for arg in method.options:
            description = getattr(type(method), arg).description
            row = [QStandardItem(arg), QStandardItem(description)]
            store.appendRow(row)
        tv.setModel(store)
        return tv
    ...
    def _setup_method_options_treeview(self):
        ...
        def get_value(column, cell, model, itr, *args):
            if hasattr(itr, 'row'):
                from PySide6.QtCore import Qt as _Qt
                option_name = model.data(model.index(itr.row(), 0), _Qt.DisplayRole)
            else:
                option_name = model.get_value(itr, 0) if hasattr(model, 'get_value') else None
            if option_name is not None:
                method = self.model.get_refinement_method()
                val = "%g" % getattr(method, option_name, 0)
                try:
                    cell.set_property("markup", val)
                except Exception:
                    cell._rendered_text = val

        def on_value_edited(rend, path, new_text, col):
            store = tv.model()
            if hasattr(store, 'get_iter'):
                ...
                try:
                    ...
                    return True
                item = store.item(row, 0)
                option_name = item.text() if item else None
            if option_name:
                method = self.model.get_refinement_method()
                try:
        ...
        if model is not None:
            if hasattr(model, 'get_iter'):
                ...
                try:
                    ...
                    return True
                idx = model.index(row, model.c_refine)
                current = model.data(idx, Qt.DisplayRole)
                model.setData(idx, not bool(current), Qt.EditRole)
        return True

    def on_btn_randomize_clicked(self, event=None):
        self.model.randomize()

    def on_auto_restrict_clicked(self, event=None):
    ...
    def on_refine_clicked(self, event=None):
