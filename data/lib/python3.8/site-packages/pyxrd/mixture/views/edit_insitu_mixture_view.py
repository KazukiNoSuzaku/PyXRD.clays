...
from PySide6.QtWidgets import QLineEdit, QComboBox, QPushButton, QGridLayout, QWidget, QHBoxLayout
from PySide6.QtCore import Qt

from pyxrd.generic.views import BaseView
from pyxrd.generic.views.validators import FloatEntryValidator
from pyxrd.mixture.views.edit_mixture_view import _find_model_row
...
class EditInSituMixtureView(BaseView):
    ...
    def reset_view(self):
        ...
        self.behav_combos = np.empty(shape=(0, 0), dtype=object)
    ...
    def add_phase_slot(self,
        ...
        layout.addWidget(new_fraction_input, r, 2)

        n_specimens = c - self.base_width
        n_new_phases = r + 1 - self.base_height
        if n_specimens > 0:
            self.phase_combos.resize((n_specimens, n_new_phases))
            self.behav_combos.resize((n_specimens, n_new_phases))
            for col in range(n_specimens):
                mcol = r - self.base_height  # new phase index
                mrow = col                   # specimen index
                ph_default = phases[mrow, mcol] if phases is not None and phases.shape[0] > mrow and phases.shape[1] > mcol else None
                bh_default = behavs[mrow, mcol] if behavs is not None and behavs.shape[0] > mrow and behavs.shape[1] > mcol else None
                self._add_new_phase_behav_combo(
                    mrow, mcol,
                    phase_store, phase_store.c_name, ph_default, phase_visible_callback, phase_combo_callback,
                    behav_store, behav_store.c_name, bh_default, behav_visible_callback, behav_combo_callback,
                )
    ...
    def add_specimen_slot(self,
        ...
        new_specimen_combo = self._get_new_combo(specimen_store, specimen_store.c_name, default=specimen, visible_callback=None, callback=specimen_callback)
        ...
        if n_phases > 0:
            ...
            self.behav_combos.resize((n_new_specimens, n_phases))
    ...
    def _add_new_phase_behav_combo(self,
        ...
        box_layout.addWidget(new_phase_combo)
        box_layout.addWidget(new_behav_combo)

        self._matrix_layout.addWidget(box, self.base_height + c, self.base_width + r)

    def _get_new_combo(self, model, text_column, default=None, visible_callback=None, callback=None, *args):
        combobox = QComboBox()
