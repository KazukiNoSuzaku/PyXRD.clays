...
class EditMixtureController(BaseController):
    ...
    def on_composition_clicked(self, widget, *args):
        ...
        dialog.add_button("Export CSV",  EXPORT_CSV)
        dialog.add_button("Export Text", EXPORT_TXT)
        ...
        while True:
            response = dialog.run()
            if response not in (EXPORT_CSV, EXPORT_TXT):
                break  # Close or window-delete

            is_csv = (response == EXPORT_CSV)
            ...
            fc.set_current_name("compositions.csv" if is_csv else "compositions.txt")
