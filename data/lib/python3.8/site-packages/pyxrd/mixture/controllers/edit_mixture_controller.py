...
class EditMixtureController(BaseController):
    ...
    def on_composition_clicked(self, widget, *args):
        ...
        for row in self.model.get_composition_matrix():
            ## PG
            row0_decoded = row[0].decode("utf-8")
            #print("row0_decoded: %s" % row0_decoded)
            row1_onwards_decoded = [x.decode("utf-8") for x in row[1:]]
            #print("row1_onwards_decoded: %s" % row1_onwards_decoded)
            comp += "%s %s\n" % (re.sub(r'(\d+)', r'<sub>\1</sub>', row0_decoded), " ".join(row1_onwards_decoded))
            ## PG
