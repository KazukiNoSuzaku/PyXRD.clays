...
class EditMixtureController(BaseController):
    ...
    def on_composition_clicked(self, widget, *args):
        ...
        # Build plain-text composition (strip Pango markup)
        lines = ["The composition of the specimens in this mixture:\n"]
        for row in self.model.get_composition_matrix():
            row0 = row[0].decode("utf-8") if isinstance(row[0], bytes) else row[0]
            rest = [
                (x.decode("utf-8") if isinstance(x, bytes) else x)
                for x in row[1:]
            ]
            element = re.sub(r'(\d+)', r'\1', row0)  # plain digits, no markup
            lines.append("%s  %s" % (element, "  ".join(rest)))
        plain_text = "\n".join(lines)
